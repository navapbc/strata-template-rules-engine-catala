...
- **Plain dict** – With plain dictionaries, the only type safety that is provided is what is provided at runtime by Marshmallow. In order to get type safety in both API and background job contexts, we'd want to re-run Marshmallow validation within service methods, [as suggested in this PR comment](https://github.com/navapbc/strata-template-rules-engine-catala/pull/51#discussion_r1054846653). The problem is that this still doesn't support static type checking, so IDE autocomplete won't be available and activities like writing tests may also become more challenging.
...
- [PR implementing this decision](https://github.com/navapbc/strata-template-rules-engine-catala/pull/57)
- [Discussion exploring options on the PR for switching from `connexion` to `apiflask`](https://github.com/navapbc/strata-template-rules-engine-catala/pull/51#discussion_r1053724252)
- [Concurrent PR exploring dataclass option](https://github.com/navapbc/strata-template-rules-engine-catala/pull/56)
- [PR exploring using TypedDict instead of dataclass](https://github.com/navapbc/strata-template-rules-engine-catala/pull/62)
